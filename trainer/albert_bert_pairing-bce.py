...
def run():
    ...
    df_train = pd.read_csv("fyp/comment_ranking_model/df_train_bce.csv")
    df_test = pd.read_csv("fyp/comment_ranking_model/df_test_bce.csv")
    ...
    # Compute the regrssion error, can ignore
    filename = f"graph_metric/{datetime.today().strftime('%Y-%m-%d-%H-%M')}_{bert_model}_{normalize}_summarize_is_{summarize}"
    compute_metric(labels_test,preds_test, filename)
    plot_graph(bert_model, normalize, labels_test, preds_test, filename)
