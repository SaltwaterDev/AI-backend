...
def evaluate_loss(net, device, criterion, dataloader):
    ...
    with torch.no_grad():
        for it, (seq, attn_masks, token_type_ids, scores) in enumerate(tqdm(dataloader)):
            seq, attn_masks, token_type_ids, scores = \
                seq.to(device), attn_masks.to(device), token_type_ids.to(device), scores.to(device)
            logits = net(seq, attn_masks, token_type_ids)
            mean_loss += criterion(logits.squeeze(-1), scores.float()).item()
            count += 1
...
def train_bert(device, net, bert_model, 
               criterion, lr, train_data,
               epochs, iters_to_accumulate, bs, maxlen):
    ...
    kf = KFold(n_splits=epochs, random_state=1, shuffle=True)

    for train_index, val_index in kf.split(train_data):
    #for ep in range(epochs):
        df_train = train_data.iloc[train_index]
        df_val = train_data.iloc[val_index]
...
def run():
    ...
    #criterions = [poissonLoss, nn.PoissonNLLLoss()]
    ...
    for criterion in criterions:
        ...
        path_to_model = train_bert(device, net, bert_model, criterion, lr, df_train, epochs, iters_to_accumulate, bs, maxlen)
        ...
        w.writelines(str(score)+'\n' for score in df_test["score"].tolist())
        w.close()
        ...
        score_test = df_test['score']  # true scores
        probs_test = pd.read_csv(path_to_output_file, header=None)[0]  # prediction probabilities
        filename = f"graph_metric/{datetime.today().strftime('%Y-%m-%d_%H-%M')}_{bert_model}_{normalize}_summarize_is_{summarize}"

        compute_metric(score_test,probs_test, filename)
        plot_graph(bert_model, normalize, score_test, probs_test, filename)
