    ...
    for ep in range(epochs):
        ...
        for it, (seq, attn_masks, token_type_ids, scores) in enumerate(tqdm(train_loader)):
            ...
            with autocast():
                ...
                # Computing loss
                loss = criterion(output.squeeze(-1), scores.float())
...
def run():
    ...
    #df_train = pd.read_csv("fyp/comment_ranking_model/df_reddit_train.csv")
    #df_test = pd.read_csv("fyp/comment_ranking_model/df_reddit_test.csv")
    
    df_train = pd.read_csv("fyp/comment_ranking_model/df_train.csv")
    df_test = pd.read_csv("fyp/comment_ranking_model/df_test.csv")
    
    print(f"size of the df_train: {df_train.shape}")
    print(f"size of the df_test: {df_test.shape}")
    ...
    train_set = CustomDataset(df_train, maxlen, bert_model=bert_model)
    print("Reading validation data...")
    val_set = CustomDataset(df_test, maxlen, bert_model=bert_model)
    ...
    #existing_net_path = "fyp/comment_ranking_model/models/roberta-large_lf_MSELoss()_lr_2e-06_val_loss_2.96311_ep_22.pt"
    #net.load_state_dict(torch.load(existing_net_path))
    net.to(device)
